...
TEST(DiyAirSensor, init) {
	...
	class InstrumentedDiyAirSensor : public DiyAirSensor {
	  public:
		InstrumentedDiyAirSensor(Serial_ * serial, SensorArray * sensors) : DiyAirSensor{serial, false} {
...
TEST(DiyAirSensor, addSensors) {
	...
	class InstrumentedDiyAirSensor : public DiyAirSensor {
	  public:
		InstrumentedDiyAirSensor(Serial_ * serial, SensorArray * sensors) : DiyAirSensor{serial} {
			// override internally-initialized array with mocked version.
			delete this->sensors;
			this->sensors = sensors;
		}
    ...
    EXPECT_CALL(sensors, add_sensor(_)).Times(Between(1, SENSORSTATE_MAX_NUM_SENSORS));
...
TEST(DiyAirSensor, loop) {
	...
	class MockedSensorDisplay :  public AirSensorDisplay {
		public:
		MOCK_METHOD(void, display_data, (SensorState * sensor_state), (override));
	} display;
	...
	class InstrumentedDiyAirSensor : public DiyAirSensor {
	  public:
		InstrumentedDiyAirSensor(Serial_ * serial,
								SensorArray * sensors,
								AirSensorDisplay * display) : DiyAirSensor{serial} {
			// override internally-initialized array and display with mocked versions.
			delete this->sensors;
			this->sensors = sensors;

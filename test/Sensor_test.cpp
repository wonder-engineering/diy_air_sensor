...
TEST(Sensor, sense) {
	class InstrumentedSensor : public Sensor {
	  public:
	    InstrumentedSensor(const char * short_name,
                    ...
                    float gain) : Sensor {short_name, accum_rate, pin, zero_adjust, gain} {}
	...
	EXPECT_CALL(sensor, read_sensor()).WillRepeatedly(Return(FAKE_SENSOR_READING));
	...
	ASSERT_EQ(FAKE_SENSOR_READING * SANE_GAIN + SANE_ZERO_ADJUST, sensor.get_last_value());
